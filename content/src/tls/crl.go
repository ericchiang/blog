...
func main() {
	...
	revoked := []pkix.RevokedCertificate{
		{SerialNumber: servCert.SerialNumber, RevocationTime: time.Now()},
	}
	now, exp := time.Now(), time.Now().Add(time.Hour)
	crlDER, err := caCert.CreateCRL(rand.Reader, caPriv, revoked, now, exp)
	if err != nil {
		log.Fatalf("creating serving cert: %v", err)
	}

	crl, err := x509.ParseCRL(crlDER)
	if err != nil {
		log.Fatalf("parsing crl: %v", err)
	}
	if err := caCert.CheckCRLSignature(crl); err != nil {
		log.Fatalf("checking crl signature: %v", err)
	}
	verifyPeerCert := func(raw [][]byte, verifiedChains [][]*x509.Certificate) error {
